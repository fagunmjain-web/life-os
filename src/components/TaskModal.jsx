...
export default function TaskModal({ task, defaultSection, defaultDate, sections, onSave, onClose }) {
  ...
  return (
    <div style={overlay} onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={modal}>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#2C2C2C', marginBottom: 18 }}>
              ...
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                {DAY_OPTIONS.map((d, i) => (
                  <button key={d} onClick={() => toggleDay(d)} style={{
                    padding: '5px 9px', fontSize: 11, fontWeight: 600, borderRadius: 7,
...
function Field({ label, children }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ fontSize: 10, fontWeight: 600, color: '#888', textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 5 }}>{label}</div>
      {children}
    </div>
  )
}
...
const typeBtn = { flex: 1, padding: '7px 0', fontSize: 12, fontWeight: 600, border: '1px solid #e0e0e0', background: '#f5f5f5', color: '#888', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit' }
const typeBtnActive = { background: '#2C2C2C', color: '#fff', borderColor: '#2C2C2C' }
